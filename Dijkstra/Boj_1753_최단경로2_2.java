...
//		isVisited = new boolean[V+1];
	...
	private static void dijk(int start) {
		...
		while(!pq.isEmpty()) {
			...
			//========= 수정 ========
			if(dist[(int)num[1]] < num[0]) continue;
			//=====================
      
//			isVisited[(int)num[1]] = true;
			Iterator<int[]> it = adj.get((int)num[1]).iterator();
			
			while(it.hasNext()) {
				int[] next = it.next();
				if(dist[next[0]] > dist[(int)num[1]] + next[1]) {
					dist[next[0]] = dist[(int)num[1]] + next[1];
